---
name: product-manual
description: 为用户已写好的 Web 平台或 Windows 桌面软件（exe）生成用户使用说明书。Web 项目会访问线上网址实际查看页面并用 puppeteer 自动截图；exe 项目读源码整理功能清单，由用户手动切换软件窗口、PowerShell 脚本截前台窗口。产出单文件 HTML（内嵌样式、可直接修改、打印友好），并可选用本机 Chrome/Edge 无头模式转成 PDF（自动回填目录页码）。当用户要求"生成使用说明书/用户手册/帮助文档"，或提到 product-manual、manual-template、html_to_pdf、screenshot 时使用。
---

# 产品使用说明书生成流程

目标：为一个已有的 Web 平台或 Windows 桌面软件（exe）生成用户说明书，**省 token**，产出易修改的 HTML，最后按需转 PDF。

## 省 token 铁律

- **绝不在对话里粘贴 HTML 全文**。所有内容直接写入文件；修改时用 Edit 做精准局部替换，不要重写整个文件。
- 样式骨架不要每次现写：直接复制 `assets/manual-template.html` 作为起点，只生成/替换内容区。
- 产品信息优先自己从代码里挖（README、路由/窗体、菜单、页面组件、package.json 等），再结合界面截图核对，只问用户代码和界面里都拿不到的东西。
...
- **Web 项目**：读 README、路由定义、侧边栏/导航组件、主要页面，整理功能模块清单和页面 URL 清单。
- **exe 桌面软件**：读源码里的窗体/界面定义（WinForms 的 `.Designer.cs`、WPF 的 `.xaml`、Qt 的 `.ui`、菜单/对话框资源等），整理功能模块和窗口清单。没有源码只有 exe 时，功能清单直接问用户。

然后**看实际界面核对**——不只看代码（菜单文案、按钮名、表格列名以实际界面为准，代码可能过时）：

- **Web 项目**：用 `scripts/screenshot.mjs` 逐页截图（脚本用法见文末「脚本」一节）：
  - 公开页面：直接截图
  - **需要登录的页面**（两步）：先 `--save-auth` 弹出真实浏览器让用户手动登录、保存登录态到 `auth.json`；之后所有内页带 `--auth auth.json` 无头截图，无需用户再参与
  - 多页面：用 `--batch` 一次截完
- **exe 桌面软件**：用 `scripts/screenshot_window.ps1` 截窗口——AI 无法替用户操作软件，**需要用户配合逐屏切换**。先把要截的界面清单列给用户，然后逐个界面发截图命令，提醒用户"已运行截图命令，请在倒计时内把软件的 XX 窗口切到前台"。展开的菜单、右键菜单、多窗口同屏等场景用 `-Full` 截全屏。

截图统一存到说明书旁的 `assets/screenshots/` 目录，命名用语义化英文（如 `dashboard.png`、`user-list.png`），后面写 HTML 时直接引用。截完用 ReadMediaFile 查看（一次并行读多张），确认界面内容。

代码里挖不到、截图也看不出的信息（产品定位、权限规则、业务约定）再问用户，用 AskUserQuestion 一次性合并提问，通常只有：
- 产品名称、面向的用户角色（管理员/普通用户…）
- Web：平台访问地址、是否需要登录截图；exe：软件如何启动、是否有需要登录/插狗/连硬件才能打开的界面
- 说明书语言（默认中文）
...
- 内容面向最终用户，写"点哪里、看到什么、注意什么"，不写实现细节。界面上的菜单名、按钮名、表格列名以实际界面截图看到的为准。
...
`scripts/screenshot_window.ps1` — exe 桌面软件截图（仅 Windows，PowerShell 自带，零依赖）：截取**前台窗口**为 PNG，AI 无法替用户操作软件，执行命令后提醒用户在倒计时内把目标窗口切到前台。用法：

```bash
powershell -ExecutionPolicy Bypass -File scripts/screenshot_window.ps1 out.png            # 倒计时 5 秒后截前台窗口
powershell -ExecutionPolicy Bypass -File scripts/screenshot_window.ps1 out.png -Delay 8   # 自定义倒计时
powershell -ExecutionPolicy Bypass -File scripts/screenshot_window.ps1 out.png -Full      # 截全屏（菜单展开、多窗口同屏等场景）
```

注意：目标窗口不能最小化；截全屏时提醒用户别把无关隐私窗口露出来。
